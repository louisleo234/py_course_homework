# -*- coding=utf-8 -*-
"""
Python第十七天练习:    IPv6工具 了解特殊的IPv6地址
日期:                 20210329
"""
...
#		null_location = ipv6_section.index('') # 找到空位，这里补@
#		ipv6_section.pop(null_location) # 把原来的空位弹出去

#		add_section = 8 - ipv6_section_len + 1 # 计算要补"0000"的个数
...
def solicited_node_multicast_address(ipv6):
	return "FF02::1:FF"  + full_ipv6(ipv6)[-7:] # -7 包括一个":", 拼接得到solicited_node_multicast_address

def mac_to_ipv6_linklocal(mac):
	# 移除多余的字符 空格, 冒号, 点, 减号
	# 转换16进制到10进制数
	mac_value = int(re.sub('[ :.-]', '', mac), 16)
	# 00:50:56:ab:4d,19作为例子
	...
	# 使用格式化打印, 转换10进制位16进制, x位16进制字符串, 并且使用02和04控制长度,并且补0
	return 'fe00::{:04x}:{:02x}ff:fe{:02x}:{:04x}'.format(high2, high1, low1, low2)

def ipv6_to_mac(ipv6):
	# 以fe80::0250:56ff:feab:4d19为例
	# 把ipv6切换为完整的ipv6地址,填充0000
	# ipv6完整地址为fe80:0000:0000:0250:56ff:feab:4d19
	...
	# 提取MAC后续部分(2个16进制位单位), 从16进制转换为10进制
	...
	# 使用格式化打印，转换10进制位16进制，x位16进制字符串, 并且使用02控制长度
